...
def get_degree_dist(FB, BA, BA_averaging=False):
    ...
    if not BA_averaging:
        ...
        axs[1, 0].plot(BA_degree_dist, "tab:red", marker="o")
        axs[1, 1].hist(BA_degree_dist, log=True, bins=50, color="tab:red")
    else:
        BA_degree_dists = []
        for _ in range(30):
            BA = nx.barabasi_albert_graph(4039, 22)
        ...
        axs[1, 0].plot(BA_avg_degree_dist, "tab:red", marker="o")
        axs[1, 1].hist(BA_avg_degree_dist, log=True, bins=50, color="tab:red")
    ...
    axs[2, 0].plot(BA_avg_degree_dist, "tab:red")

    axs[2, 1].hist(FB_degree_dist, log=True, bins=50)
    axs[2, 1].hist(BA_avg_degree_dist, log=True, bins=50, color='tab:red')
