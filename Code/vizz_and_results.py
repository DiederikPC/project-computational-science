...
def get_graph(parameter,model):
    ...
    for i in range(3):           
        ...
        axs[first_axis[i], second_axis[i]].plot(data_m.iloc[:,0],ba_mean,label = 'BA',color = 'brown')
        ...
        alpha = 0.2, color = 'brown', edgecolor = 'brown')
...
results = pd.DataFrame({'model':[],'parameter':[],'metric':[],'avg_perc_error':[]})
